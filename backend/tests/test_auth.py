...
class TestGetHospitals:
    def test_returns_list(self, client, seed_hospital_and_user):
        res = client.get("/api/v1/qc/auth/hospitals")
        assert res.status_code == 200
        data = res.json()
        assert isinstance(data, list)
        assert len(data) >= 1
        assert data[0]["qc_name"] == "TestHospital"
...
class TestLogin:
    ...
    def test_missing_fields(self, client):
        res = client.post("/api/v1/qc/auth/login", json={"email": "a@b.com"})
...
class TestTokenValidation:
    ...
    def test_protected_endpoint_invalid_token(self, client):
        res = client.get("/api/v1/qc/doctor/sessions", headers={"Authorization": "Bearer invalidtoken"})
        assert res.status_code == 401
