...
class MRMCStudy(Base):
    ...
    hospital_id = Column(String(20), ForeignKey("qc_hospitals.qc_id"), nullable=False)
    created_by = Column(Integer, ForeignKey("qc_users.qc_id"), nullable=False)
    created_at = Column(TIMESTAMP, server_default=text("CURRENT_TIMESTAMP"))
...
class MRMCStudyParticipant(Base):
    ...
    user_id = Column(Integer, ForeignKey("qc_users.qc_id"), nullable=False)
...
class MRMCStudySubject(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    study_id = Column(Integer, ForeignKey("mrmc_studies.id", ondelete="CASCADE"), nullable=False)
    patient_session_id = Column(String(20), ForeignKey("qc_patient_sessions.qc_id"), nullable=False)
    is_included = Column(Boolean, nullable=False, default=True)
    reader_user_id = Column(Integer, ForeignKey("qc_users.qc_id"), nullable=True)
    arbiter_user_id = Column(Integer, ForeignKey("qc_users.qc_id"), nullable=True)
    created_at = Column(TIMESTAMP, server_default=text("CURRENT_TIMESTAMP"))
